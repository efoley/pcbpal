...
function renderExportResult(result: ExportResult): void {
  if (isInteractive()) {
    ...
    if (result.correctionsApplied > 0) {
      clack.log.info(`Rotation corrections applied to ${result.correctionsApplied} components`);
    }
  } else {
...
export function registerProductionCommand(program: Command): void {
  const prod = program.command("production").description("Configure and export production files");
  ...
  prod
    ...
    .action(async (opts: { output?: string; fromJlcpcb?: boolean; useDrillOrigin?: boolean }) => {
      try {
